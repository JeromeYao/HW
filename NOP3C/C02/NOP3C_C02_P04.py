#!/usr/bin/env python3
# Filename:NOP3C_C02_P04.py

__author__ = 'JeromeYao'

'''
It's a note of 4th paragraph of 2nd chapter of python3-cookbook chinese version
https://github.com/JeromeYao/python3-cookbook/blob/master/source/c02/p04_match_and_search_text.rst
'''
...
# match() 总是从字符串开始去匹配，如果你想查找字符串任意部分的模式出现位置，使用 findall() 方法去替代。
...
# 使用re模块进行匹配和搜索文本的核心步骤就是先使用 re.compile() 编译正则表达式字符串，
...
# 最后，如果你仅仅是做一次简单的文本匹配/搜索的话，可以略过编译部分，直接使用 re 模块级别的函数。
print(re.findall(r'(\d+)/(\d+)/(\d+)', text))

# 但是需要注意的是，如果你打算做大量的匹配和搜索操作的话，最好先编译正则表达式，然后再重复使用它。
# 模块级别的函数会将最近编译过的模式缓存起来，因此并不会消耗太多的性能， 但是如果使用预编译模式的话，
# 你将会减少查找和一些额外的处理损耗。
